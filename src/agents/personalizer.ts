...
export async function personalizeGaps(answers: Answers, gaps: Gap[]): Promise<Gap[]> {
  ...
  try {
    ...
    return gaps.map((g) => {
      ...
      return rewritten && rewritten.trim().length > 20 ? { ...g, explanationAr: rewritten.trim() } : g;
    });
  } catch (err) {
    console.warn('[personalizer] gap rewrite failed, keeping templates:', err instanceof Error ? err.message : err);
    return gaps;
  }
}
...
export async function personalizeWarnings(answers: Answers, warnings: string[]): Promise<string[]> {
  ...
  try {
    ...
    console.warn('[personalizer] warning rewrite failed, keeping originals:', err instanceof Error ? err.message : err);
    return warnings;
  }
}
...
function companyContextLines(answers: Answers): string[] {
  ...
  if (answers.q5_dpo_appointed) {
    lines.push(`حالة DPO: ${answers.q5_dpo_appointed === 'yes' ? 'مُعيَّن' : answers.q5_dpo_appointed === 'no' ? 'غير مُعيَّن' : 'غير معروف'}`);
  }
...
function userScaleLabel(v: NonNullable<Answers['q4_user_count']>): string {
  switch (v) {
    case 'under_10k': return 'أقل من ١٠ آلاف';
    case '10k_100k':  return 'بين ١٠ آلاف و ١٠٠ ألف';
    case 'over_100k': return 'أكثر من ١٠٠ ألف';
  }
}
