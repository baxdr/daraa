...
function OperationalSummary({ report }: { report: OperationalReport }) {
  ...
  const healthColor =
    health >= 75 ? 'text-accent-strong' :
    health >= 50 ? 'text-warn-strong'   :
                   'text-danger';
  ...
  return (
    <section className="grid gap-10 md:grid-cols-3 md:gap-8">
      <div>
        ...
        <div className={`mt-3 font-display text-6xl font-extrabold leading-none tabular-nums tracking-tighter md:text-7xl ${healthColor}`}>
          {health}<span className="text-2xl md:text-3xl text-muted">٪</span>
        </div>
...
function CountBlock({
  ...
  return (
    <div>
      ...
      <div className={`mt-3 font-display text-6xl font-extrabold leading-none tabular-nums tracking-tighter md:text-7xl ${color}`}>
...
function TimelineRow({ gap }: { gap: OperationalGap }) {
  ...
  const pct = isOverdue
    ? 100
    : Math.max(0, Math.min(100, (clamped / TIMELINE_WINDOW_DAYS) * 100));
  const barColor =
    gap.severity === 'critical' ? 'bg-danger'   :
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-4">
        ...
        <div className={`shrink-0 font-display text-lg font-extrabold tabular-nums ${
          isOverdue ? 'text-danger' : gap.severity === 'medium' ? 'text-warn-strong' : 'text-ink-2'
        }`}>
          {days === 0 ? (
      ...
      <div className="relative mt-3 h-1.5 overflow-hidden bg-paper-2">
        {isOverdue && (
          <div
            aria-hidden
            className="absolute inset-y-0 start-0 w-1/6 bg-danger/40"
          />
        )}
        <div
          aria-hidden
  ...
  return (
    <section>
      ...
      <div className="space-y-5">
        {critical.length > 0 && <AlertGroup title="حرج — تحرّك الآن" items={critical} tone="danger" />}
