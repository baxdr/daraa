...
/**
 * Scan-progress theater. Live agent timeline with both activity events
 * (status updates) and A2A messages (inter-agent dialogue).
 *
 * The UI is honest about the agent layering (§ "3 LLM-driven + 7 specialists"
 * per the v4 design) — it shows the specialists that actually spoke during
 * this run, not a padded set.
 */
...
export function ScanProgress({ scanId }: { scanId: string }) {
    ...
    async function poll() {
      ...
      try {
        ...
        if (data.status === 'complete') {
          timer = setTimeout(() => {
            if (!cancelledRef.current) router.replace(`/scan/${scanId}/report`);
          }, 1100);
          return;
        }
        ...
        if (attempts >= MAX_ATTEMPTS) {
          setError('استغرق الفحص وقت أطول من اللازم.');
          return;
        }
  ...
  const progress = useMemo(() => {
    ...
    return Math.min(100, Math.round((completed / STATION_ROW.length) * 100));
  }, [stationStatus]);

  return (
    <main className="mx-auto max-w-3xl px-6 py-10 md:px-10 md:py-16">
      <header className="mb-8">
        <span className="eyebrow">الفحص جارٍ</span>
        <h1 className="mt-3 font-display text-4xl font-extrabold leading-tight tracking-tight md:text-5xl">
          الوكلاء يتواصلون الآن
        </h1>
        <p className="mt-3 max-w-xl text-base text-ink-2">
          وكيل البحث (LLM) يجلب آخر التحديثات، وكيل الفحص يزور موقعك، متخصّص حماية البيانات
          يطابق القواعد، ووكيل التحليل يُنتج النتيجة — شوفوهم يتبادلون المعلومات مباشرةً.
        </p>
      </header>

      <div className="rule mb-8" />

      {/* Stations — show which agents spoke this run */}
      <section className="mb-10 grid grid-cols-2 gap-0 overflow-hidden border border-rule bg-white sm:grid-cols-5">
        {STATION_ROW.map((s, i) => (
          <Station
            key={s.id}
            label={s.label}
            state={stationStatus[s.id] ?? 'idle'}
            isLast={i === STATION_ROW.length - 1}
          />
        ))}
      </section>
      ...
      {status !== 'complete' && status !== 'error' && (
        <p className="mt-12 text-[11px] text-muted">
          معرّف الفحص: <code dir="ltr" className="font-mono">{scanId}</code>
        </p>
      )}
    </main>
  );
...
function Station({
  ...
  return (
    <div
      className={`flex items-center gap-2.5 border-rule px-3 py-3 ${
        isLast ? '' : 'sm:border-s'
